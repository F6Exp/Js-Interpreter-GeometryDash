...
function spawnPlatform() {
    ...
    if (Math.random() < 0.5) {
        ...
        var position = Math.floor(Math.random() * 3); // 0=left, 1=middle, 2=right
        var spikeX;
        if (position === 0) spikeX = x; // left
        else if (position === 1) spikeX = x + Math.floor((width - sz.w) / 2); // middle
        else spikeX = x + width - sz.w; // right
...
setTextSize(2);
drawString("by github.com/F6Exp", 20, 70);
setTextSize(1);
